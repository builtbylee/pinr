...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onAuthenticated }) => {
    ...
    const handleAppleSignIn = async () => {
        ...
        try {
            ...
            console.error('[AuthScreen] Error type:', typeof e);
            console.error('[AuthScreen] Error message:', e.message);
            console.error('[AuthScreen] Error code:', e.code);
            console.error('[AuthScreen] Full error:', JSON.stringify(e, Object.getOwnPropertyNames(e)));
            console.error('[AuthScreen] ==========================================');
            
            // Show detailed error in alert for debugging
            if (__DEV__) {
                Alert.alert(
                    'Apple Sign-In Error (Debug)',
                    `Message: ${e.message || 'Unknown'}\n\nCode: ${e.code || 'N/A'}\n\nFull: ${JSON.stringify(e, Object.getOwnPropertyNames(e)).substring(0, 200)}`,
                    [{ text: 'OK' }]
                );
            }
            
            if (e.message !== 'Sign-in was cancelled') {
                setError(e.message || 'The authorization attempt failed for an unknown reason');
            }
        } finally {
            setIsLoading(false);
        }
    ...
    const renderForm = (isLogin: boolean) => (
        <View style={styles.contentContainer}>
            ...
            <TouchableOpacity
                ...
                disabled={isLoading}
            >
                    <View style={{ width: 24, height: 24, justifyContent: 'center', alignItems: 'center', marginLeft: -6 }}>
...
const styles = StyleSheet.create({
    ...
    appleButton: {
        ...
        shadowRadius: 4,
        elevation: 3,
        paddingLeft: 0, // Match Google button padding
    },
    appleButtonText: {
        color: '#FFFFFF',
        fontSize: 16,
